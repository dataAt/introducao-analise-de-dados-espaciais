...
No entanto, nem tudo são flores e tão simples quanto mostramos. Os métodos apresentados possuem diversos parametros que ajudam a lidar com dados mal formatados e ruidosos, então, salientamos que nosso exemplo é de cunho acadêmico. Como diz um dos criadores do *Tidyverse*:
...
`sf` para a leitura, escrita e manipução de dados espaciais. # Introdução à Geociências `r emo::ji("world")`
...
Observe que, diferentemente dos tipos geométricos da classe `sfg`, a classe `sfc` apresenta diferentes características, como a projeção `proj4string`, ID do sistema de referência geográfica `epsg`, *bolding box* `bbox` e a dimensão `dimension`.
...
- `sf` Junção das classes `sfg` e `sfc`. Contêm atributos correspondes ao tipo geométricos, bem como atributos discretos, categóricos que representam características do objeto.
...
- `Contains` - Verifica se a geométria B contêm;
...
Para saber se duas geométrias são iguais, usa-se o métodos `st_equals`:

```{r}
  # Verifica se duas geométrias são iguais
...
> Para verificar se duas geométricas são exatamente iguais, use `st_equals_exact`.
...
```{r}
  ...
  # Criação dos tipos geométricos
  polygon_1 <- sf::st_polygon(list(poligono_1))
  poligono_2 <- sf::st_polygon(list(poligono_2))
```
...
```{r, echo=FALSE}
  ...
  plot(sf::st_as_sfc(sf::st_as_text(poligono_2)), border = "blue", axes = FALSE, main="LINESTRING", lwd = 2,  add=TRUE, reset=FALSE, xlim =xlim, ylim = ylim,col=alpha("grey", 0.35))
...
O método de conversão não faz transformação das coordenadas geográficas. Para fazer a transformação, podemos usar o métodod `st_transform`:
