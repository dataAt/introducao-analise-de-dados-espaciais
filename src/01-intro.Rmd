...
Uma das características presentes nas estruturas matriciais é a resolução espacial, que corresponde a um valor que corresponde a dimensionalidade linear em um espaço geográfico. *“Quanto menor a dimensão das células, maior a resolução da imagem matricial e, consequente, maior a quantidade de memória necessária para armazená-la”* @lisboa2001modelagem.
...
Os tipos de dados espaciais dentro do OGC *Simple Features* estão divididos em dezesseis tipos de diferentes instâncias, podendo ser divididos em outros três grupos: **Primitivos**, **Múltiplos** e **Coleções**.  No tipo **primitivo** temos os seguintes objetos:
