...
Python é uma linguagem de programação versátil, multiparadigma, que permite aos desenvolvedores criar soluções rápidas para seus problemas sem grandes preocupações com a sintaxe e formas de uso. 

Tais características fazem Python alcançar os mais variados nichos de aplicação. Por ser uma linguagem geral para atingir tais nichos, foram criadas pela comunidade de Python diversas bibliotecas, todas seguindo a ideia da simplicidade de aplicação e prototipação da linguagem. É neste contexto que surge o GeoPandas, que fornece extensões espaciais para o Pandas, uma biblioteca de análise de dados muito conhecida na comunidade Python.

Desta forma, este capítulo busca explicar as principais formas de utilização do GeoPandas. Por ser criada sob o Pandas, a próxima seção faz uma pequena revisão de uso dessa biblioteca. É recomendado que o leitor tenha ideias gerais sobre a biblioteca Pandas, para que os conceitos apresentados aqui sejam mais facilmente compreendidos, caso necessário, existe o material do  [Curso de introdução à análise de dados](https://dataat.github.io/introducao-analise-de-dados/introducao.html), onde são apresentados os principais conceitos de uso do Pandas e de outras bibliotecas, não deixe de conferir!
...
Perceba que, as `Series` possuem o campo para o armazenamento dos dados (Representado em Azul), e o índice (Representado em roxo), não podendendo ser adicionado nenhum outro campo, isto faz com que, todos os dados tenham de ser armazenados em apenas uma coluna com várias linhas.

Para o `DataFrame` o cenário apresentado na Figura é diferente, além dos índices, há também as colunas (Representada em vermelho), o que permite que dentro desta estrutura existam não só um conjunto de linhas, como as `Series` mas sim um conjunto de colunas, onde cada uma dessas possuem várias linhas. Na prática, é tido que um `DataFrame` é um conjunto de `Series`.
...
Bom, agora que já entendemos a utilização básica do Pandas, vamos começar a falar um pouco sobre o GeoPandas `r emo::ji("purple_heart")`
...
Olha que interessante! Para cada estado, há seu nome, a região a que pertence e o código desta UF. Agora que já sabemos como estão nossos dados, vamos voltar um pouco na coluna `geometry`.
...
A área é devolvida na unidade de medida da projeção espacial utilizada no conjunto de dados.
...
O atributo `centroid` devolve centro geométrico de uma geometria. Sua representação é feita na Figura abaixo.
...
Para fazermos um pequeno experimento, vamos pegar a figura com os estados e fazer seu plot junto com os centroids.
...
O método `buffer` cria um circulo de influência no entorno de uma localização, e pode ser aplicado em diferentes tipos de análises espaciais. Sua representação é feita na Figura abaixo.
...
O método `contains` verifica se uma geometria contém outra, sua representação visual é apresentada abaixo .

<center>
![](res/3_python/opgeom_contains.svg)
</center>

Vamos fazer um teste da operação com o estado do Amazonas, verificando se sua geometria está contida nos dados de estados do Brasil.
...
Por fim, na parte de seleção através dos dados espaciais, vamos testar o método `intersects`, que verifica se uma geometria faz intersecção com outra. A operação pode ser vista abaixo.
...
Este foi o capítulo sobre GeoPandas e suas facilidades de uso `r emo::ji("star2")`. Cabe lembrar que aqui são apresentados apenas os conceitos iniciais, há muito mais para explorar, com uma gama de outros operadores e conceitos a serem vistos, então, não deixe de consultar a documentação oficial e fazer seus testes `r emo::ji("dizzy")`.
